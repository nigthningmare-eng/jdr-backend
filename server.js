...
app.patch('/api/pnjs/:id', async (req, res) => {
  try {
    const id = req.params.id;
    const currentRes = await pool.query('SELECT data FROM pnjs WHERE id = $1', [id]);
    if (!currentRes.rows.length) return res.status(404).json({ message: 'PNJ non trouvé.' });

    const current = currentRes.rows[0].data;
    const incoming = req.body || {};
    const locks = new Set(current.lockedTraits || []);
    for (const f of locks) {
      if (f in incoming) delete incoming[f];
    }
